...
class NegativeProtoGuard:
    ...
    def _derive_base_dataset_name(self) -> str:
        """Derive the base-class training dataset name from the configured train split."""
        novel_ds = self.cfg.DATASETS.TRAIN[0]

        # MFDC / VOC patterns:
        #   removevoc_2007_trainval_all1_1shot_seed0 -> voc_2007_trainval_base1
        #   voc_2007_trainval_all1_1shot_seed0       -> voc_2007_trainval_base1
        #   voc_2007_trainval_novel1_1shot_seed0     -> voc_2007_trainval_base1
        m = re.match(r"(?:remove)?voc_(\d+)_trainval_(?:all|novel)(\d+)_.*", novel_ds)
        if m:
            year, sid = m.groups()
            return f"voc_{year}_trainval_base{sid}"

        # Legacy VOC pattern kept for compatibility.
