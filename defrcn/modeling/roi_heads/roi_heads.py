    ...
    def generate_features(self, gt_classes):
        ...
        for c in self.novel_index:
            ...
            univariate_normal_dists = distributions.normal.Normal(
                calibrated_mean, scale=torch.sqrt(calibrated_std.clamp(min=1e-8)))
            ...
            if self.memory:
                with torch.no_grad():
                    ...
                    pad_size = self.cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE \
                        * self.cfg.SOLVER.IMS_PER_BATCH // ( torch.distributed.get_world_size() if torch.distributed.is_initialized() else 1)
